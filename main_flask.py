...
def answers_manage() -> Response:

    if request.method == "POST":

        try:

            answers: AnsewrsModel = AnsewrsModel(**request.json)
        ...
        try:
            ...
            elif token == ــTOKEN__:

                data: List[QuestsAndAnswers] = database.session.query(QuestsAndAnswers).all()

                answers_list = list()

                for answers in data:
                    answers: QuestsAndAnswers
